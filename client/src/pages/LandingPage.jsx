...
const FEATURES = [
  { icon: <Calendar size={20} />, title: "Office Hours", desc: "Browse and reserve available office hour slots from professors and TAs, so no more back-and-forth emails." },
  { icon: <Users size={20} />, title: "Group Meetings", desc: "Coordinate group sessions with availability polling. The best time gets selected and everyone is notified." },
  { icon: <MessageSquare size={20} />, title: "Meeting Requests", desc: "Send a one-on-one meeting request directly to a professor or TA. They accept or decline, and you're notified." },
];
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Eyebrow label */}
        <PillBadge className="mc-anim-0">McGill University · SOCS</PillBadge>
        {/* Headline */}
        <h1
          className="mc-anim-1"
          style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {FEATURES.map((f, i) => (
            <FeatureCard key={i} icon={f.icon} title={f.title} desc={f.desc} delay={i * 0.07} />
          ))}
        </div>
      </div>
